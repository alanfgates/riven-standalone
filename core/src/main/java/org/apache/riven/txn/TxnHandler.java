...
import org.apache.riven.annotation.RetrySemantics;
import org.apache.riven.conf.MetastoreConf;
import org.apache.riven.conf.MetastoreConf.ConfVars;
import org.apache.riven.impl.DatabaseProduct;
import org.apache.riven.impl.ThreadPool;
...
import org.apache.riven.utils.StringableMap;
 ...
 * {@link TxnUtils#txnIdToString(long)} and {@link TxnUtils#lockIdToString(long)} in all messages.
 ...
 * and {@link org.apache.riven.impl.RetryingHMSHandler}.  The retry logic there is very
...
abstract public class TxnHandler implements TxnStore, TxnStore.MutexAPI {
  ...
  public void setConf(Configuration conf) {
    ...
    synchronized (TxnHandler.class) {
      if (connPool == null) {
        ...
        // Also schedule the open txn counter
      ...
      try {
        ...
        if (abortTxns(dbConn, Collections.singletonList(txnid), true) != 1) {
          ...
          if(status == TxnStatus.ABORTED) {
            LOG.info("abortTxn(" + TxnUtils.txnIdToString(txnid) +
      ...
      try {
        ...
        if (lockHandle == null) {
          ...
          if(actualTxnStatus == TxnStatus.COMMITTED) {
             ...
             */
            LOG.info("Nth commitTxn(" + TxnUtils.txnIdToString(txnid) + ") msg");
            return;
          }
        ...
        if (rs.next()) {
          ...
          if (rs.next()) {
            //found a conflict
            String committedTxn = "[" + TxnUtils.txnIdToString(rs.getLong(1)) + "," + rs.getLong(2) +
                "]";
            ...
            String msg = "Aborting [" + TxnUtils.txnIdToString(txnid) + "," + rs.getLong(6) + "]" +
        ...
        if ((modCount = stmt.executeUpdate(s)) < 1) {
          ...
          LOG.info("Expected to move at least one record from txn_components to " +
            "completed_txn_components when committing txn! " + TxnUtils.txnIdToString(txnid));
        }
              ...
              switch (lc.getOperationType()) {
                ...
                default:
                  ...
                  throw new IllegalStateException("Unexpected DataOperationType: " + lc.getOperationType()
                    + " agentInfo=" + rqst.getAgentInfo() + " " + TxnUtils.txnIdToString(txnid));
              }
            }
  ...
  public LockResponse checkLock(CheckLockRequest rqst)
    ...
    try {
      ...
      try {
        ...
        if(info == null) {
          throw new NoSuchLockException("No such lock " + TxnUtils.lockIdToString(extLockId));
        }
        if (info.txnId > 0) {
        ...
        throw new MetaException("Unable to update transaction database " +
          TxnUtils.lockIdToString(extLockId) + " " + StringUtils.stringifyException(e));
      } finally {
        closeDbConn(dbConn);
        unlockInternal();
      }
        ...
        if (rc < 1) {
          ...
          if(info == null) {
            ...
            LOG.info("No lock in " + LOCK_WAITING + " mode found for unlock(" +
              TxnUtils.lockIdToString(rqst.getLockid()) + ")");
  ...
  /**
   * used to sort entries in {@link org.apache.riven.api.ShowLocksResponse}
   */
  ...
  private static void shouldNeverHappen(long txnid) {
    throw new RuntimeException("This should never happen: " + TxnUtils.txnIdToString(txnid));
  }
  private static void shouldNeverHappen(long txnid, long extLockId, long intLockId) {
    throw new RuntimeException("This should never happen: " + TxnUtils.txnIdToString(txnid) + " "
      + TxnUtils.lockIdToString(extLockId) + " " + intLockId);
  }
   ...
   * @throws TxnHandler.RetryException when the operation should be retried
   */
   ...
   * @throws org.apache.riven.api.MetaException if the time cannot be determined
   */
  ...
  private static class LockInfo {
    ...
    @Override
    public String toString() {
      return TxnUtils.lockIdToString(extLockId) + " intLockId:" +
        intLockId + " " + TxnUtils.txnIdToString(txnId)
  ...
  private LockResponse checkLock(Connection dbConn, long extLockId)
    ...
    try {
      ...
      LOG.debug("checkLock(): Setting savepoint. extLockId=" + TxnUtils.lockIdToString(extLockId));
      ...
      if(!writeSet.isEmpty()) {
        if(writeSet.get(0).txnId == 0) {
          ...
          throw new IllegalStateException("Found Write lock for " + TxnUtils.lockIdToString(extLockId) + " but no txnid");
        }
        stmt = dbConn.createStatement();
        ...
        if(rs.next()) {
          ...
          String msg = "Aborting " + TxnUtils.txnIdToString(writeSet.get(0).txnId) +
            " since a concurrent committed transaction [" + TxnUtils.txnIdToString(rs.getLong(4)) + "," + rs.getLong(5) +
  ...
  private void acquire(Connection dbConn, Statement stmt, long extLockId, LockInfo lockInfo)
    ...
    if (rc < 1) {
      ...
      dbConn.rollback();
      throw new NoSuchLockException("No such lock: (" + TxnUtils.lockIdToString(extLockId) + "," +
        + lockInfo.intLockId + ") " + TxnUtils.txnIdToString(lockInfo.txnId));
    }
  ...
  private static void raiseTxnUnexpectedState(TxnStatus actualStatus, long txnid) 
    ...
    switch (actualStatus) {
      case ABORTED:
        throw new TxnAbortedException("Transaction " + TxnUtils.txnIdToString(txnid) + " already aborted");
      case COMMITTED:
        throw new NoSuchTxnException("Transaction " + TxnUtils.txnIdToString(txnid) + " is already committed.");
      case UNKNOWN:
        throw new NoSuchTxnException("No such transaction " + TxnUtils.txnIdToString(txnid));
      case OPEN:
        throw new NoSuchTxnException(TxnUtils.txnIdToString(txnid) + " is " + TxnStatus.OPEN);
      default:
  ...
  private static void ensureValidTxn(Connection dbConn, long txnid, Statement stmt)
    ...
    if (rs.getString(1).charAt(0) == TXN_ABORTED) {
      LOG.debug("Going to rollback");
      rollbackDBConn(dbConn);
      throw new TxnAbortedException("Transaction " + TxnUtils.txnIdToString(txnid) +
  ...
  private LockInfo getTxnIdFromLockId(Connection dbConn, long extLockId)
    ...
    try {
      ...
      LOG.debug("getTxnIdFromLockId(" + extLockId + ") Return " + TxnUtils.txnIdToString(info.txnId));
      return info;
    } finally {
      close(rs);
      closeStmt(stmt);
    ...
    try {
      ...
      if (!sawAtLeastOne) {
        throw new MetaException("This should never happen!  We already " +
          "checked the lock(" + TxnUtils.lockIdToString(extLockId) + ") existed but now we can't find it!");
      }
      return ourLockInfo;
    } finally {
      closeStmt(stmt);
  ...
  public void performTimeOuts() {
    ...
    try {
      ...
      while(true) {
        ...
        Counter abortCntr = Metrics.getOrCreateCounter(MetricsConstants.NUM_TIMED_OUT_TXNS);
        if (abortCntr != null) abortCntr.inc(numTxnsAborted);
  ...
  private static synchronized DataSource setupJdbcConnectionPool(Configuration conf,
                                                                 int maxPoolSize,
    ...
    String connectionPooler =
        MetastoreConf.getVar(conf, ConfVars.CONNECTION_POOLING_TYPE).toLowerCase();
  ...
  static final class SQLGenerator {
    ...
    SQLGenerator(DatabaseProduct dbProduct, Configuration conf) {
  ...
  private static String getMetastoreJdbcPasswd(Configuration conf) throws SQLException {
    try {
