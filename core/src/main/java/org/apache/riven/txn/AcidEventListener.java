...
public class AcidEventListener extends MetaStoreEventListener {
  ...
  private TxnStore getTxnHandler() {
    boolean hackOn = MetastoreConf.getBoolVar(conf, MetastoreConf.ConfVars.HIVE_IN_TEST) ||
        MetastoreConf.getBoolVar(conf, MetastoreConf.ConfVars.HIVE_IN_TEZ_TEST);
    ...
    if (hackOn) {
      origTxnMgr = MetastoreConf.getVar(conf, MetastoreConf.ConfVars.HIVE_TXN_MANAGER);
      origConcurrency = MetastoreConf.getBoolVar(conf, MetastoreConf.ConfVars.HIVE_SUPPORT_CONCURRENCY);
    }
    ...
    // Set them back
    if (hackOn) {
      MetastoreConf.setVar(conf, MetastoreConf.ConfVars.HIVE_TXN_MANAGER, origTxnMgr);
      MetastoreConf.setBoolVar(conf, MetastoreConf.ConfVars.HIVE_SUPPORT_CONCURRENCY, origConcurrency);
    }
