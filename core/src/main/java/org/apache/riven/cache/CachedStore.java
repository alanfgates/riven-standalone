...
import org.apache.riven.impl.Deadline;
...
public class CachedStore implements RawStore, Configurable {
  ...
  public void setConf(Configuration conf) {
    String rawStoreClassName = MetastoreConf.getVar(conf, MetastoreConf.ConfVars.CACHED_RAW_STORE_IMPL,
  ...
  static class CacheUpdateMasterWork implements Runnable {
    ...
    public void run() {
      ...
      String rawStoreClassName =
          MetastoreConf.getVar(cachedStore.conf, ConfVars.CACHED_RAW_STORE_IMPL,
              ObjectStore.class.getName());
      try {
        RawStore rawStore =
            JavaUtils.getClass(rawStoreClassName, RawStore.class).newInstance();
    ...
    private void updateTablePartitions(RawStore rawStore, String dbName, String tblName) {
      try {
        ...
        if (partitionCacheLock.writeLock().tryLock()) {
          ...
          SharedCache.refreshPartitions(StringUtils.normalizeIdentifier(dbName),
    ...
    private void updateTableColStats(RawStore rawStore, String dbName, String tblName) {
      try {
        ...
        if (tableColStatsCacheLock.writeLock().tryLock()) {
          ...
          SharedCache.refreshTableColStats(StringUtils.normalizeIdentifier(dbName),
              StringUtils.normalizeIdentifier(tblName), tableColStats.getStatsObj());
        }
      } catch (MetaException | NoSuchObjectException e) {
    ...
    private void updateTablePartitionColStats(RawStore rawStore, String dbName, String tblName) {
      try {
        ...
        if (partitionColStatsCacheLock.writeLock().tryLock()) {
          ...
          SharedCache.refreshPartitionColStats(StringUtils.normalizeIdentifier(dbName),
  ...
  public boolean getPartitionsByExpr(String dbName, String tblName, byte[] expr,
    ...
    Table table = SharedCache.getTableFromCache(StringUtils.normalizeIdentifier(dbName), StringUtils.normalizeIdentifier(tblName));
  ...
  public static List<String> partNameToVals(String name) {
    ...
    for (String kv : kvp) {
      vals.add(HdfsUtils.unescapePathName(kv.substring(kv.indexOf('=') + 1)));
    }
    return vals;
  }
    ...
    for (String colName : colNames) {
      ColumnStatisticsObj colStat =
          mergeColStatsForPartitions(StringUtils.normalizeIdentifier(dbName),
              StringUtils.normalizeIdentifier(tblName), partNames, colName);
      if (colStat == null) {
