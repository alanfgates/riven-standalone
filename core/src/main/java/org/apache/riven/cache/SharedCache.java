 ...
 */
package org.apache.riven.cache;
...
import org.apache.riven.impl.StatObjectConverter;
...
public class SharedCache {
  ...
  private static Map<String, CachedStore.TableWrapper> tableCache = new TreeMap<String, CachedStore.TableWrapper>();
  private static Map<String, CachedStore.PartitionWrapper> partitionCache =
      new TreeMap<String, CachedStore.PartitionWrapper>();
  ...
  private static Map<String, ColumnStatisticsObj> tableColStatsCache =
      new TreeMap<String, ColumnStatisticsObj>();
  private static Map<ByteArrayWrapper, CachedStore.StorageDescriptorWrapper> sdCache =
      new HashMap<ByteArrayWrapper, CachedStore.StorageDescriptorWrapper>();
  ...
  public static synchronized Table getTableFromCache(String dbName, String tableName) {
    CachedStore.TableWrapper tblWrapper = tableCache.get(CacheUtils.buildKey(dbName, tableName));
    if (tblWrapper == null) {
      return null;
    }
  ...
  public static synchronized void addTableToCache(String dbName, String tblName, Table tbl) {
    ...
    CachedStore.TableWrapper wrapper;
    if (tbl.getSd() != null) {
      byte[] sdHash = MetaStoreUtils.hashStorageDescriptor(tbl.getSd(), md);
      StorageDescriptor sd = tbl.getSd();
      increSd(sd, sdHash);
      tblCopy.setSd(null);
      wrapper = new CachedStore.TableWrapper(tblCopy, sdHash, sd.getLocation(), sd.getParameters());
    } else {
      wrapper = new CachedStore.TableWrapper(tblCopy, null, null, null);
    }
  ...
  public static synchronized void removeTableFromCache(String dbName, String tblName) {
    CachedStore.TableWrapper tblWrapper = tableCache.remove(CacheUtils.buildKey(dbName, tblName));
  ...
  public static synchronized List<Table> listCachedTables(String dbName) {
    ...
    for (CachedStore.TableWrapper wrapper : tableCache.values()) {
  ...
  public static synchronized void addPartitionToCache(String dbName, String tblName, Partition part) {
    ...
    CachedStore.PartitionWrapper wrapper;
    if (part.getSd()!=null) {
      byte[] sdHash = MetaStoreUtils.hashStorageDescriptor(part.getSd(), md);
      StorageDescriptor sd = part.getSd();
      increSd(sd, sdHash);
      partCopy.setSd(null);
      wrapper = new CachedStore.PartitionWrapper(partCopy, sdHash, sd.getLocation(), sd.getParameters());
    } else {
      wrapper = new CachedStore.PartitionWrapper(partCopy, null, null, null);
    }
  ...
  public static synchronized Partition getPartitionFromCache(String key) {
    CachedStore.PartitionWrapper wrapper = partitionCache.get(key);
    if (wrapper == null) {
      return null;
    }
  ...
  public static synchronized List<Partition> listCachedPartitions(String dbName, String tblName, int max) {
    ...
    int count = 0;
    for (CachedStore.PartitionWrapper wrapper : partitionCache.values()) {
  ...
  public static void increSd(StorageDescriptor sd, byte[] sdHash) {
    ...
    if (sdCache.containsKey(byteArray)) {
      ...
      sdCache.put(byteArray, new CachedStore.StorageDescriptorWrapper(sdToCache, 1));
    }
  }

  public static void decrSd(byte[] sdHash) {
    ByteArrayWrapper byteArray = new ByteArrayWrapper(sdHash);
    CachedStore.StorageDescriptorWrapper sdWrapper = sdCache.get(byteArray);
    sdWrapper.refCount--;
  ...
  public static StorageDescriptor getSdFromCache(byte[] sdHash) {
    CachedStore.StorageDescriptorWrapper sdWrapper = sdCache.get(new ByteArrayWrapper(sdHash));
  ...
  public static synchronized void refreshPartitions(String dbName, String tblName,
    ...
    LOG.debug("CachedStore: updating cached partition objects for database: " + dbName
        + " and table: " + tblName);
    Iterator<Entry<String, CachedStore.PartitionWrapper>> iterator = partitionCache.entrySet().iterator();
    while (iterator.hasNext()) {
      CachedStore.PartitionWrapper partitionWrapper = iterator.next().getValue();
