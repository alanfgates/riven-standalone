...
import org.apache.riven.metrics.PerfLogger;
...
public class RetryingHMSHandler implements InvocationHandler {
  ...
  private RetryingHMSHandler(Configuration hiveConf, IHMSHandler baseHandler, boolean local) throws MetaException {
    this.origConf = hiveConf;
    this.baseHandler = baseHandler;
    if (local) {
      baseHandler.setConf(hiveConf); // tests expect configuration changes
      // applied directly
      // to metastore
    }
  ...
  public Result invokeInternal(final Object proxy, final Method method, final Object[] args) throws Throwable {
    ...
    boolean reloadConf = MetastoreConf.getBoolVar(origConf,
        MetastoreConf.ConfVars.HMSHANDLERFORCERELOADCONF);
    long retryInterval = MetastoreConf.getTimeVar(origConf,
        MetastoreConf.ConfVars.HMSHANDLERINTERVAL, TimeUnit.MILLISECONDS);
    int retryLimit = MetastoreConf.getIntVar(origConf,
        MetastoreConf.ConfVars.HMSHANDLERATTEMPTS);
  ...
  private Configuration getActiveConf() {
    return activeConf;
  }
}
