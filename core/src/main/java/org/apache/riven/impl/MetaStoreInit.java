...
import org.apache.riven.hooks.JDOConnectionURLHook;
...
public class MetaStoreInit {
   ...
   */
  public static boolean updateConnectionURL(Configuration originalConf, Configuration activeConf,
    ...
    if (connectUrl != null && !connectUrl.equals(currentUrl)) {
      LOG.error(
          String.format("Overriding %s with %s",
              ...
              connectUrl));
      MetastoreConf.setVar(activeConf, MetastoreConf.ConfVars.CONNECTURLKEY,
          connectUrl);
      return true;
    }
    return false;
  ...
  synchronized private static void initConnectionUrlHook(Configuration conf,
    ...
    String className =
        MetastoreConf.getVar(conf, MetastoreConf.ConfVars.CONNECTURLHOOK, "").trim();
    if (className.equals("")) {
