...
import static org.apache.riven.utils.MetaStoreUtils.validateName;
import static org.apache.riven.utils.MetaStoreUtils.validatePartitionNameCharacters;
...
import org.apache.riven.AlterHandler;
...
import org.apache.riven.listeners.SessionPropertiesListener;
import org.apache.riven.listeners.TransactionalValidationListener;
...
import org.apache.riven.listeners.events.InsertEvent;
...
import org.apache.riven.listeners.events.PreAuthorizationCallEvent;
...
import org.apache.riven.listeners.events.PreReadDatabaseEvent;
import org.apache.riven.listeners.events.PreReadTableEvent;
import org.apache.riven.messaging.EventMessage.EventType;
...
import org.apache.riven.txn.AcidCompactionHistoryService;
import org.apache.riven.txn.AcidEventListener;
import org.apache.riven.txn.AcidOpenTxnsCounterService;
import org.apache.riven.txn.AcidWriteSetService;
import org.apache.riven.txn.TxnStore;
import org.apache.riven.txn.TxnUtils;
import org.apache.hadoop.security.UserGroupInformation;
import org.apache.hadoop.util.ReflectionUtils;
import org.apache.riven.utils.CommonCliOptions;
import org.apache.riven.utils.HdfsUtils;
import org.apache.riven.utils.JavaUtils;
import org.apache.riven.utils.LogUtils;
...
public class MetaStoreServer extends ThriftHiveMetastore {
  ...
  public static class HMSHandler extends FacebookBase implements IHMSHandler, ThreadLocalRawStore {
    ...
    public Configuration getMsConf() {
      return conf;
    }
    ...
    @Override
    public List<MetaStoreEventListener> getTransactionalListeners() {
    ...
    public void init() throws MetaException {
      initListeners = MetaStoreUtils.getMetaStoreListeners(
          MetaStoreInitListener.class, conf,
          MetastoreConf.getVar(conf, ConfVars.INIT_HOOKS));
      ...
      // TODO RIVEN Build a default alter handler and use it here.
      ...
      if (registry != null && MetastoreConf.getBoolVar(conf, ConfVars.INIT_METADATA_COUNT_ENABLED)) {
        synchronized (HMSHandler.class) {
      ...
      preListeners = MetaStoreUtils.getMetaStoreListeners(MetaStorePreEventListener.class,
          conf,
          MetastoreConf.getVar(conf, ConfVars.PRE_EVENT_LISTENERS));
      ...
      transactionalListeners = MetaStoreUtils.getMetaStoreListeners(MetaStoreEventListener.class,
      ...
      endFunctionListeners = MetaStoreUtils.getMetaStoreListeners(
          ...
          MetastoreConf.getVar(conf, ConfVars.END_FUNCTION_LISTENERS));
      ...
      long cleanFreq = MetastoreConf.getTimeVar(conf, ConfVars.EVENT_CLEAN_FREQ,
    ...
    private static RawStore newRawStoreForConf(Configuration conf) throws MetaException {
      ...
      String rawStoreClassName = MetastoreConf.getVar(confCopy, ConfVars.RAW_STORE_IMPL);
      LOG.info(addPrefix("Opening raw store with implementation class:" + rawStoreClassName));
      return RawStoreProxy.getProxy(confCopy, conf, rawStoreClassName, threadLocalId.get());
    }
    ...
    private void createDefaultDB_core(RawStore ms) throws MetaException, InvalidObjectException {
      try {
        ...
        db.setOwnerName(MSConstants.PUBLIC);
    ...
    private void createDefaultRoles_core() throws MetaException {
      ...
      try {
        ms.addRole(MSConstants.PUBLIC, MSConstants.PUBLIC);
      } catch (InvalidObjectException e) {
        LOG.debug(MSConstants.PUBLIC + " role already exists",e);
      } catch (NoSuchObjectException e) {
        // This should never be thrown.
        LOG.warn("Unexpected exception while adding "+ MSConstants.PUBLIC +" roles" , e);
      }
      LOG.info("Added "+ MSConstants.PUBLIC+ " role in metastore");
    ...
    private void create_type_core(final RawStore ms, final Type type)
      ...
      if (!org.apache.riven.utils.MetaStoreUtils.validateName(type.getName(), null)) {
    ...
    private void create_table_core(final RawStore ms, final Table tbl,
        ...
        throws AlreadyExistsException, MetaException,
        InvalidObjectException, NoSuchObjectException {
      if (!org.apache.riven.utils.MetaStoreUtils.validateName(tbl.getTableName(), conf)) {
      ...
      try {
        ...
        if (tbl.getParameters() == null ||
            tbl.getParameters().get(metastoreConstants.DDL_TIME) == null) {
          tbl.putToParameters(metastoreConstants.DDL_TIME, Long.toString(time));
        }
    ...
    */
    ...
    public void truncate_table(final String dbName, final String tableName, List<String> partNames)
      ...
      try {
        ...
        for (Path location : getLocationsForTruncate(getMS(), dbName, tableName, tbl, partNames)) {
          FileSystem fs = location.getFileSystem(getMsConf());
          if (!HdfsUtils.isPathEncrypted(getMsConf(), fs.getUri(), location)) {
            HdfsUtils.HadoopFileStatus status = new HdfsUtils.HadoopFileStatus(getMsConf(), fs, location);
            FileStatus targetStatus = fs.getFileStatus(location);
            String targetGroup = targetStatus == null ? null : targetStatus.getGroup();
            wh.deleteDir(location, true, isAutopurge);
            fs.mkdirs(location);
            HdfsUtils.setFullFileStatus(getMsConf(), status, targetGroup, fs, location, false);
          } else {
            FileStatus[] statuses = fs.listStatus(location, HdfsUtils.HIDDEN_FILES_PATH_FILTER);
    ...
    private Partition append_partition_common(RawStore ms, String dbName, String tableName,
      ...
      try {
        ...
        validatePartitionNameCharacters(part_vals, partitionValidationPattern);
        ...
        part.putToParameters(metastoreConstants.DDL_TIME, Long.toString(time));
    ...
    private boolean startAddPartition(
      ...
      validatePartitionNameCharacters(part.getValues(),
    ...
    private void initializeAddedPartition(
      ...
      if (part.getParameters() == null ||
          part.getParameters().get(metastoreConstants.DDL_TIME) == null) {
        part.putToParameters(metastoreConstants.DDL_TIME, Long.toString(time));
      }

      // Inherit table properties into partition properties.
      Map<String, String> tblParams = tbl.getParameters();
      String inheritProps = MetastoreConf.getVar(conf, ConfVars.PART_INHERIT_TBL_PROPS)
          .trim();
    ...
    public DropPartitionsResult drop_partitions_req(
      ...
      try {
        ...
        for (Partition part : parts) {
          ...
          firePreEvent(new PreDropPartitionEvent(tbl, part, deleteData, this));
          if (colNames != null) {
            partNames.add(HdfsUtils.makePartName(colNames, part.getValues()));
          }
    ...
    private void rename_partition(final String db_name, final String tbl_name,
      ...
      try {
        ...
        if (part_vals != null && !part_vals.isEmpty()) {
          validatePartitionNameCharacters(new_part.getValues(), partitionValidationPattern);
        }
    ...
    public void alter_index(final String dbname, final String base_table_name,
      ...
      newIndex.putToParameters(metastoreConstants.DDL_TIME, Long.toString(System
    ...
    private void alter_table_core(final String dbname, final String name, final Table newTable,
      ...
      if (newTable.getParameters() == null ||
          newTable.getParameters().get(metastoreConstants.DDL_TIME) == null) {
        newTable.putToParameters(metastoreConstants.DDL_TIME, Long.toString(System
            .currentTimeMillis() / 1000));
      }
    ...
    public String get_config_value(String name, String defaultValue)
      ...
      try {
        ...
        try {
          toReturn = conf.get(name, defaultValue);
    ...
    private Index add_index_core(final RawStore ms, final Index index, final Table indexTable)
      ...
      try {
        ...
        index.putToParameters(metastoreConstants.DDL_TIME, Long.toString(time));
    ...
    public boolean grant_role(final String roleName,
      ...
      firePreEvent(new PreAuthorizationCallEvent(this));
      if (MSConstants.PUBLIC.equals(roleName)) {
        throw new MetaException("No user can be added to " + MSConstants.PUBLIC +". Since all users implictly"
        + " belong to " + MSConstants.PUBLIC + " role.");
      }
      Boolean ret = null;
      try {
        RawStore ms = getMS();
    ...
    public boolean create_role(final Role role)
      ...
      firePreEvent(new PreAuthorizationCallEvent(this));
      if (MSConstants.PUBLIC.equals(role.getRoleName())) {
         throw new MetaException(MSConstants.PUBLIC + " role implictly exists. It can't be created.");
      }
      Boolean ret = null;
      try {
    ...
    public boolean drop_role(final String roleName)
      ...
      firePreEvent(new PreAuthorizationCallEvent(this));
      if (ADMIN.equals(roleName) || MSConstants.PUBLIC.equals(roleName)) {
        throw new MetaException(MSConstants.PUBLIC + "," + ADMIN + " roles can't be dropped.");
      }
      Boolean ret = null;
      try {
    ...
    private boolean revoke_role(final String roleName, final String userName,
      ...
      firePreEvent(new PreAuthorizationCallEvent(this));
      if (MSConstants.PUBLIC.equals(roleName)) {
        throw new MetaException(MSConstants.PUBLIC + " role can't be revoked.");
      }
      Boolean ret = null;
      try {
        RawStore ms = getMS();
    ...
    private void validateFunctionInfo(Function func) throws InvalidObjectException, MetaException {
      if (!org.apache.riven.utils.MetaStoreUtils.validateName(func.getFunctionName(), null)) {
  ...
  public static IHMSHandler newRetryingHMSHandler(IHMSHandler baseHandler, Configuration hiveConf)
      throws MetaException {
  ...
  private static void startHouseKeeperService(Configuration conf) throws Exception {
    ...
    RunnableConfigurable acidHousekeeper = new AcidOpenTxnsCounterService();
    acidHousekeeper.setConf(conf);
    ThreadPool.getPool().scheduleAtFixedRate(acidHousekeeper,
        MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.TIMEDOUT_TXN_REAPER_START, TimeUnit.MILLISECONDS),
        MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.TIMEDOUT_TXN_REAPER_INTERVAL, TimeUnit.MILLISECONDS),
        TimeUnit.MILLISECONDS);
    RunnableConfigurable compactionHistory = new AcidCompactionHistoryService();
    compactionHistory.setConf(conf);
    ThreadPool.getPool().scheduleAtFixedRate(compactionHistory, 0,
        MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.COMPACTOR_HISTORY_REAPER_INTERVAL, TimeUnit.MILLISECONDS),
        TimeUnit.MILLISECONDS);
    RunnableConfigurable writeSetCleaner = new AcidWriteSetService();
    writeSetCleaner.setConf(conf);
    ThreadPool.getPool().scheduleAtFixedRate(compactionHistory, 0,
        MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.WRITE_SET_REAPER_INTERVAL, TimeUnit.MILLISECONDS),
        TimeUnit.MILLISECONDS);
  }
