...
import org.apache.riven.messaging.EventMessage;
...
/**
 * Default implementation of alter
 */
public class DefaultAlterHandler implements AlterHandler {

  protected Configuration conf;
  private static final Logger LOG = LoggerFactory.getLogger(DefaultAlterHandler.class);
    ...
    try {
      ...
      if (rename
          ...
          && (oldt.getSd().getLocation().compareTo(newt.getSd().getLocation()) == 0
            || org.apache.commons.lang.StringUtils.isEmpty(newt.getSd().getLocation()))
        ...
        if (!tableInSpecifiedLoc) {
          ...
          if (!HdfsUtils.equalsFileSystem(srcFs, destFs)) {
        ...
        if (MetaStoreUtils.requireCalStats(conf, null, null, newt, environmentContext) &&
            !isPartitionedTable) {
          Database db = msdb.getDatabase(newDbName);
  ...
  public Partition alterPartition(final RawStore msdb, Warehouse wh, final String dbname,
    ...
    if (new_part.getParameters() == null ||
        new_part.getParameters().get(metastoreConstants.DDL_TIME) == null ||
        Integer.parseInt(new_part.getParameters().get(metastoreConstants.DDL_TIME)) == 0) {
      new_part.putToParameters(metastoreConstants.DDL_TIME, Long.toString(System
    ...
    if (part_vals == null || part_vals.size() == 0) {
      try {
        ...
        if (MetaStoreUtils.requireCalStats(conf, oldPart, new_part, tbl, environmentContext)) {
    ...
    try {
      ...
      for (Partition tmpPart: new_parts) {
        ...
        if (tmpPart.getParameters() == null ||
            tmpPart.getParameters().get(metastoreConstants.DDL_TIME) == null ||
            Integer.parseInt(tmpPart.getParameters().get(metastoreConstants.DDL_TIME)) == 0) {
          tmpPart.putToParameters(metastoreConstants.DDL_TIME, Long.toString(System
              .currentTimeMillis() / 1000));
        }
        ...
        if (MetaStoreUtils.requireCalStats(conf, oldTmpPart, tmpPart, tbl, environmentContext)) {
