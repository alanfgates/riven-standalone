...
import org.apache.riven.impl.ReplChangeManager;
...
public class TestReplChangeManager {
  ...
  public static void setUp() throws Exception {
    ...
    conf.setInt(CommonConfigurationKeysPublic.FS_TRASH_INTERVAL_KEY, 60);
    warehouse = new Warehouse(conf);
    fs = new Path(cmroot).getFileSystem(conf);
    try {
  ...
  private void createFile(Path path, String content) throws IOException {
    FSDataOutputStream output = path.getFileSystem(conf).create(path);
  ...
  public void testRecyclePartTable() throws Exception {
    ...
    SerDeInfo serdeInfo = new SerDeInfo("LBCSerDe",
        "org.apache.hadoop.hive.serde2.columnar.LazyBinaryColumnarSerDe", new HashMap<>());
    ...
    Assert.assertTrue(part1Path.getFileSystem(conf).exists(part1Path));
    Assert.assertTrue(part2Path.getFileSystem(conf).exists(part2Path));
    Assert.assertTrue(part3Path.getFileSystem(conf).exists(part3Path));
    ...
    Path cmPart1Path = ReplChangeManager.getCMPath(part1Path, conf, path1Chksum);
    Assert.assertTrue(cmPart1Path.getFileSystem(conf).exists(cmPart1Path));

    // Verify dropPartition recycle part files
    client.dropPartition(dbName, tblName, Arrays.asList("20160102"));
    Assert.assertFalse(part2Path.getFileSystem(conf).exists(part2Path));
    Path cmPart2Path = ReplChangeManager.getCMPath(part2Path, conf, path2Chksum);
    Assert.assertTrue(cmPart2Path.getFileSystem(conf).exists(cmPart2Path));

    // Verify dropTable recycle partition files
    client.dropTable(dbName, tblName);
    Assert.assertFalse(part3Path.getFileSystem(conf).exists(part3Path));
    Path cmPart3Path = ReplChangeManager.getCMPath(part3Path, conf, path3Chksum);
    Assert.assertTrue(cmPart3Path.getFileSystem(conf).exists(cmPart3Path));
  ...
  public void testRecycleNonPartTable() throws Exception {
    ...
    Assert.assertTrue(filePath1.getFileSystem(conf).exists(filePath1));
    Assert.assertTrue(filePath2.getFileSystem(conf).exists(filePath2));
    Assert.assertTrue(filePath3.getFileSystem(conf).exists(filePath3));

    ReplChangeManager cm = ReplChangeManager.getInstance(conf);
    // verify cm.recycle(Path) api moves file to cmroot dir
    cm.recycle(filePath1, false);
    Assert.assertFalse(filePath1.getFileSystem(conf).exists(filePath1));

    Path cmPath1 = ReplChangeManager.getCMPath(filePath1, conf, fileChksum1);
    Assert.assertTrue(cmPath1.getFileSystem(conf).exists(cmPath1));

    // Verify dropTable recycle table files
    client.dropTable(dbName, tblName);

    Path cmPath2 = ReplChangeManager.getCMPath(filePath2, conf, fileChksum2);
    Assert.assertFalse(filePath2.getFileSystem(conf).exists(filePath2));
    Assert.assertTrue(cmPath2.getFileSystem(conf).exists(cmPath2));

    Path cmPath3 = ReplChangeManager.getCMPath(filePath3, conf, fileChksum3);
    Assert.assertFalse(filePath3.getFileSystem(conf).exists(filePath3));
    Assert.assertTrue(cmPath3.getFileSystem(conf).exists(cmPath3));
  ...
  public void testClearer() throws Exception {
    FileSystem fs = warehouse.getWhRoot().getFileSystem(conf);
    ...
    ReplChangeManager.getInstance(conf).recycle(dirTbl1, false);
    ReplChangeManager.getInstance(conf).recycle(dirTbl2, false);
    ReplChangeManager.getInstance(conf).recycle(dirTbl3, true);

    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part11, conf, fileChksum11)));
    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part12, conf, fileChksum12)));
    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part21, conf, fileChksum21)));
    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part22, conf, fileChksum22)));
    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part31, conf, fileChksum31)));
    Assert.assertTrue(fs.exists(ReplChangeManager.getCMPath(part32, conf, fileChksum32)));

    fs.setTimes(ReplChangeManager.getCMPath(part11, conf, fileChksum11), now - 86400*1000*2, now - 86400*1000*2);
    fs.setTimes(ReplChangeManager.getCMPath(part21, conf, fileChksum21), now - 86400*1000*2, now - 86400*1000*2);
    fs.setTimes(ReplChangeManager.getCMPath(part31, conf, fileChksum31), now - 86400*1000*2, now - 86400*1000*2);
    fs.setTimes(ReplChangeManager.getCMPath(part32, conf, fileChksum32), now - 86400*1000*2, now - 86400*1000*2);
    ...
    do {
      ...
      if (!fs.exists(ReplChangeManager.getCMPath(part11, conf, fileChksum11)) &&
          fs.exists(ReplChangeManager.getCMPath(part12, conf, fileChksum12)) &&
          !fs.exists(ReplChangeManager.getCMPath(part21, conf, fileChksum21)) &&
          fs.exists(ReplChangeManager.getCMPath(part22, conf, fileChksum22)) &&
          !fs.exists(ReplChangeManager.getCMPath(part31, conf, fileChksum31)) &&
          !fs.exists(ReplChangeManager.getCMPath(part31, conf, fileChksum31))) {
        cleared = true;
      }
    } while (!cleared);
